...
//      AIC merchant realm login page with a callbackUrl pointing back to /checkout.
//   2. Exchanges the merchant access_token from the Auth.js session for an payment
//      realm token via getPaymentToken, then fetches the shopper's saved wallet
//      cards from payment-api using the payment token as Bearer.
//   3. Passes walletCards and userId to CheckoutForm for the interactive UI.
//
// payment-api validates tokens against the payment realm JWKS only; a merchant token
// would be rejected with 401. getPaymentToken performs the full Step 1 exchange
...
export default async function CheckoutPage() {
  ...
  // Exchange the merchant access_token for an payment realm token so the wallet
  // fetch is accepted by payment-api (which validates against the payment realm
  ...
  try {
    const res = await fetch(
      ...
      { headers: { Authorization: `Bearer ${downstreamToken}` }, cache: 'no-store' },
    )
    if (res.ok) {
