...
// with the app directory (`apps/payment-api`) as cwd, so `../../data` is the
// repo `data/` dir regardless of where the compiled module ends up.
 ...
 * cwd (which is `apps/payment-api` under both `next dev` and `next start`).
 */
