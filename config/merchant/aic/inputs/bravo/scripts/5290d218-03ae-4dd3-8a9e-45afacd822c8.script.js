...
(function () {
  ...
  utils.setScopeClaimsMap({
    profile: ['name', 'family_name', 'given_name', 'zoneinfo', 'locale', 'orgs', 'groups'],
    email: ['email'],
    address: ['address'],
    phone: ['phone_number'],
    secctx: ['orgs', 'groups', 'username', 'account_status'],
  ...
  utils.setClaimResolvers({
    ...
    orgs: utils.getUserProfileClaimResolver('fr-attr-multi1'),
    username: utils.getUserProfileClaimResolver('uid'),
    account_status: utils.getUserProfileClaimResolver('inetUserStatus'),
    post_logout_url: function () {
      return 'https://openam-volker-dev.forgeblocks.com/login/?realm=/alpha'
    },
    ip_address: function () {
      return session.getProperty("Host")
    },
    groups: utils.getUserGroupsClaimResolver(),
    falco: function () {
      return 'mapped to openid scope'
    },
  });
  ...
  function getUtils() {
    ...
    function setScopeClaimsMap(params) {
      ...
      logger.error('setScopeClaimsMap(): '+JSON.stringify(params));
    }
    ...
    function setClaimResolvers(params) {
      ...
      logger.error('setClaimResolvers(): Done.');
    }
    ...
    function getUserProfileClaimResolver(attributeName) {
      ...
      function resolveClaim(claim) {
        logger.error('resolveClaim(): Resolving claim: '+claim.getName());
        var userProfileValue;

        if (identity) {
          var realAttrName = attributeName;
          var customSet = new java.util.HashSet();
          var isCustomAttr = attributeName.startsWith('custom_');
          logger.error('resolveClaim(): Resolving claim for attribute: '+attributeName);
          if (isCustomAttr) {
            realAttrName = 'fr-idm-custom-attrs';
            var customAttrValue;
            try {
              customAttrValue = JSON.parse(String(identity.getAttribute(realAttrName).toArray()[0]))[attributeName];
            } catch (error) { }
            if (customAttrValue) {
              customSet.add(customAttrValue);
              userProfileValue = getClaimValueFromSet(claim, customSet);
            }
          } else {
            userProfileValue = getClaimValueFromSet(claim, identity.getAttribute(realAttrName));
          }
          ...
          if (userProfileValue && !userProfileValue.isEmpty()) {
            if (!claim.getValues() || claim.getValues().isEmpty() || claim.getValues().contains(userProfileValue)) {
              logger.error('resolveClaim(): Resolved claim for attribute: '+attributeName+' to value: '+userProfileValue+' from user profile.');
              return userProfileValue;
            }
          }
        } else if (session) {
          logger.error('resolveClaim(): No identity, using session');
          var customAttrValue;
          var customSet = new java.util.HashSet();
          try {
            customAttrValue = session.getProperty(attributeName);
          } catch (error) {
            logger.error('resolveClaim(): Error retrieving '+attributeName+' from session: '+error);
          }
          if (customAttrValue) {
            customSet.add(customAttrValue);
            userProfileValue = getClaimValueFromSet(claim, customSet);
          ...
          if (userProfileValue && !userProfileValue.isEmpty()) {
            if (!claim.getValues() || claim.getValues().isEmpty() || claim.getValues().contains(userProfileValue)) {
              logger.error('resolveClaim(): Resolved claim for attribute: '+attributeName+' to value: '+userProfileValue+' from user session.');
              return userProfileValue;
            }
          }
        }
        logger.error('resolveClaim(): Unable to resolve claim: '+claim.getName());
      }

      return resolveClaim;
    }

    function getUserGroupsClaimResolver() {
      function resolveClaim(claim) {
        var fr = JavaImporter(
    ...
    function getEssentialClaimResolver(resolveClaim) {
      ...
      function resolveEssentialClaim(claim) {
        ...
        if (claim.isEssential() && !isClaimValueValid(claimValue)) {
          throw new frJava.InvalidRequestException(
            'Could not provide value for essential claim: ' + claim.getName()
          );
        }
    ...
    function resolveAnyClaim(claim) {
      logger.error('resolveAnyClaim(): claim: '+claim.getName());
    ...
    function getClaimValueFromSet(claim, set) {
      if (set && set.size()) {
        ...
        logger.warning(
          'OIDC Claims script. Got an empty set for claim: ' + claim.getName()
        );
      }
    }
    ...
    function getUserInfoClaims() {
      logger.error('getUserInfoClaims()');
      return new frJava.UserInfoClaims(
        getComputedClaims(),
        getCompositeScopes()
      );
    }
    ...
    function getComputedClaims() {
      ...
      function computeClaim(claim) {
        ...
        try {
          logger.error('computeClaim(): Computing claim: '+claim.getName());
          resolveClaim = claimResolvers[claim.getName()] || resolveAnyClaim;
          logger.error('computeClaim(): Using claim resolver: '+resolveClaim);

          return resolveClaim(claim);
        } catch (e) {
          message =
            'OIDC Claims script exception. Unable to resolve OIDC Claim. ' + e + '\n' + e.stack;
    ...
    return {
      ...
      getUserGroupsClaimResolver: getUserGroupsClaimResolver,
