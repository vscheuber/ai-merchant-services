...
The merchant runtime now treats the merchant JWT `sub` as the customer identity metadata only. It looks up `alpha_user` by the pair `custom_merchantId` and `custom_merchantCustomerId`, creates users through IDM's auto-ID create endpoint without supplying `_id`, and generates a separate UUID `userName`. A create response is followed by a pair lookup; a `409` is not accepted as success unless the pair is immediately readable. Non-2xx lookup/create responses remain failures and are not converted into a broad fallback.
