...
## Task 10 — RESOLVED: live end-to-end verification through payment-api

Getting a real browser session through Task 9's journey design *and* the full downstream chain (Step 2 exchange, `payment-api` call) surfaced several tenant-configuration and AM-behavior gotchas — each initially indistinguishable from a real blocker until traced to its actual cause. Full detail, including exact fixes and a doc citation, is in the cross-project tracker (Section 10); summary:

- **Trusted-issuer config must match the token's *actual* `iss`** (the merchant IDP's custom-domain-alias issuer, confirmed via that alias's own `/.well-known/openid-configuration`), not the canonical tenant hostname — and `unreasonableLifetimeLimit` is a hard ceiling on `exp - iat` in seconds, not a lenient check. Both failures surface as the same generic `"Invalid Merchant Token"` branch, indistinguishable without decoding the actual token.
- **AI Agent client secrets are unrecoverable after creation** — this repo's own provisioner (`buildAIAgentCreatePayload`) deliberately strips `userpassword` before every AI Agent create call, and it is never returned on read either. Rotation requires `frodo agent ai export` → add a plain-string `coreOAuth2ClientConfig.userpassword` (unwrapped, unlike sibling fields) → `frodo agent ai import`.
- **AIC's AI Agent "Acting On Behalf Of" privilege model requires an explicit `audience` param** (the target Application's own OAuth2 client ID) on the RFC 8693 token-exchange request. Omitting it produces a generic `HTTP 400 invalid_request` with no useful detail in the response — the real cause (`BadRequestException: This delegation is not permitted`, thrown from `TokenExchangeGrantTypeHandler.verifyWithPrivilegeModel`) is visible only in AM's own server logs via `cloud.log.fetch`.
- **A realm's `tokenSigningAlgorithm` setting gates whether JWKS-based local JWT verification can work at all.** This tenant's `/alpha` realm is configured for symmetric (HS256) stateless tokens — JWKS only ever publishes asymmetric keys, so no JWKS URI configuration can make local verification succeed. RFC 7662 introspection is required instead, and the introspecting client needs the `am-introspect-all-tokens` scope to introspect tokens issued to a *different* client — without it, AM silently returns `{"active": false}` for a valid token rather than an error.
- The `com.sun.identity.idm.server.IdServicesImpl` `"Unable to find entry: ...AgentService..."` WARN log lines that accompany essentially every identity resolution in this tenant are benign noise (a legacy LDAP plugin with no IDM-backed entries) — not diagnostic of anything, despite superficially looking like a real datastore misconfiguration.
...
The application and Northwind OAuth2/AI Agent desired-state work is present. The custom payment-provider user schema attributes are live (Task 9). The merchant-subject-to-payment-UUID mapping is resolved and **live-verified end to end** (Task 10): `merchant-token-login` is provisioned, the runtime Step 1 flow lives entirely in `apps/chatbot-agent` (silent SSO + journey + session bridge), and a real browser session was confirmed all the way through Step 2 and a `payment-api` call returning real shopper data. Remaining: retiring the pre-rename `poc-jwt-login` journey (dormant provisioner flag exists, not yet invoked — left in place pending the team's own confirmation window).
